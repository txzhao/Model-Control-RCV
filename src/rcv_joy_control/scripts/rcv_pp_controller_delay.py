...
class Translator:
	...
	def statecallback(self, data): 
		...
		self.linear_v = self.ref_v + random.uniform(-1, 1)
		if cur_counter == 0:
			self.linear_v = 0
		self.linear_v = max(0, self.linear_v)
		if cur_counter > 0:
			cur_counter -= 1
		fb_index = int(round(cur_counter/20.0*self.ref_v/self.points_per_meter))
		self.x = Path[fb_index, 0] + self.x0
		self.y = Path[fb_index, 1] + self.y0
		self.yaw = Path[fb_index, 2]
		self.counter += 1
	...
	def pathControl(self):
		...
		print("x: {5}, y: {6}, dist1: {7} dist: {0}, desire_point: {1}, current_point {2}, kappa: {3}, velocity: {4}"
			.format(dist, desire, index, command.kappa, self.linear_v, self.x, self.y, dist_list[index]))
		#print("kappa: {0}, velocity: {1}".format(command.kappa, rcv_v))
